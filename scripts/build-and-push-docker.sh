...
echo ""
# Auto-update terraform.tfvars and commit
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TFVARS="${SCRIPT_DIR}/../terraform/terraform.tfvars"

echo "📝 Updating terraform/terraform.tfvars..."
sed -i '' "s|container_image  = \".*\"|container_image  = \"${FULL_IMAGE_NAME}\"|g" "${TFVARS}"
echo "✅ terraform.tfvars updated"

echo ""
echo "📝 Committing and pushing to trigger TFC deployment..."
git -C "${SCRIPT_DIR}/.." add terraform/terraform.tfvars
git -C "${SCRIPT_DIR}/.." commit -m "chore: update container image to ${IMAGE_TAG}"
git -C "${SCRIPT_DIR}/.." push origin dev
echo "✅ Pushed — TFC will deploy automatically"
echo ""
